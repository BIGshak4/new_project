...
5. **"I saw it at company X."** `POST /v1/questions/{key}/sightings` with a company name; slugged, one row per (question, user, company); `QuestionSummary.companies` carries `{slug, name, count}` (never who); `GET /v1/questions?company=` and `GET /v1/companies` search by company. Table `question_sighting` = migration `20260923000000_question_sightings.sql` (dry-run OK, **not applied**): until then readers return empty and the writer answers `503 temporarily_unavailable`, which the button shows as "company tags are opening soon".
...
8. **Full verification from scratch:** `docs/system-verification-2026-09-24.md`.
...
**Morning list (needs Shaked):** (a) apply `20260923000000_question_sightings.sql` and `20260923000001_interview_answer_images.sql` (`scripts/dry_run_sql.py … --replaces "policy learners upload answer images"` for the second), then flip `INTERVIEW_PHOTOS` to true; (b) rotate the Netlify build hook pasted in chat and update the GitHub secret `NETLIFY_PRACTICE_BUILD_HOOK`; (c) decide on the Opus 5.5 comparison; (d) open the site once through the goal card and the progress page and say what feels off.
...
| Date | Change |
|---|---|
| 2026-09-24 | Overnight (§5k): the goal (job type, interview date, minutes a day) on `user_profile`; six job types that re-weight the role's skills for the plan, the next question and the interview; questions listed by job relevance and by company; "I saw it at company X" (migration `20260923000000`, pending approval); progress = overview in words + road-so-far graph + plan until the interview; practice feedback → follow-up → next question under the answer; drawn circuits in interview answers (photos wait for migration `20260923000001`, pending approval); visual refresh; 706 offline tests; `scripts/e2e_goal_and_visuals.py`; verification report `docs/system-verification-2026-09-24.md` |
