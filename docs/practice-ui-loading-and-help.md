...
- Question preview leads with the actual question, followed by a single “Write my answer” action. An inline link near the title jumps to that action for long questions. New practice uses one answer with progressive hints, without a mode/confidence questionnaire or automatic follow-up questions. See [the guided practice handoff](guided-practice-and-assistant-handoff.md) for the current editor and assessment contract. Active attempts keep hint and solution buttons near the question title; they remain separate, explicit exposure actions.
