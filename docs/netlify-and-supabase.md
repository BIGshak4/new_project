...
The practice frontend is connected to Shaked's authenticated FastAPI service at https://jobrun-api.onrender.com. Attempts, submissions and engine progress persist in Supabase. The provider remains `scripted`: feedback and skill metrics are visibly marked as simulated. No real AI key has been configured by this integration. Self-assessment stays separate. Expanding to 150 reviewed questions is still a content milestone.
...
Still pending: Shaked's inbox confirmation, custom SMTP for external testers, GitHub auto-deployment, real model credentials and expert feedback validation, expanded reviewed content, and broader user testing. No custom domain, payment provider, native application or outgoing reminder system is configured. See [the integration handoff](pilot-integration-handoff.md) for the current checks and next steps.
