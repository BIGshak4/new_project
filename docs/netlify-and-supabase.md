...
The practice frontend is connected to Shaked's authenticated FastAPI service at https://jobrun-api.onrender.com. Attempts, submissions and engine progress persist in Supabase. Shaked has enabled the real Anthropic provider (Opus evaluator, Sonnet feedback/tips); the live health check confirms it. The server-only Storage key is configured and `/health.answer_images` reports `assessed`; private-image fetching was verified. Human review must still validate model interpretation of handwriting and diagrams. Self-assessment stays separate. Expanding to 150 reviewed questions is still a content milestone.
...
The 20 September deployment verification passed 33 web tests, 610 backend offline tests and all 16 live database tests. The deployed API smoke checks and private-image fetch test also passed. See the repair report for exact revisions and limitations.

Still pending: custom SMTP for external testers, joint expert feedback validation, expanded reviewed content, and broader user testing. The Render backend is already in Frankfurt; its Free instance still sleeps when idle. No custom domain, payment provider, native application or outgoing reminder system is configured. See [the real-model handback](p1-real-model-handback.md) and [the deployment repair report](deployment-repair-2026-09-20.md).
