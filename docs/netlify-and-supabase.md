...
Still pending: custom SMTP for external testers, joint expert feedback validation, expanded reviewed content, and broader user testing. The live database rerun is tracked in the deployment handoff. The Render backend is already in Frankfurt; its Free instance still sleeps when idle. No custom domain, payment provider, native application or outgoing reminder system is configured. See [the real-model handback](p1-real-model-handback.md) and [the deployment repair report](deployment-repair-2026-09-20.md).
