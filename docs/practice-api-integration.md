...
Added 2026-09-26 (**grade first**): a submission now answers as soon as the answer is scored and saved (typically the evaluator's 5–6 s instead of 16–37 s). `SubmissionView.feedback_pending = true` means the band, summary, key points, check, evidence and XP are final, while `card`, `tip` and the follow-up's wording are still being written (a few seconds). `AttemptView.feedback_pending` is true while any revision is in that state, and `FollowUpView.question_pending` marks a follow-up that is decided but has no words yet (its `question` is empty; answering it is refused with `409 follow_up_not_ready`). Poll `GET` the attempt until `feedback_pending` is false. Writing the words never changes a result. If the server restarts before the words are saved, the next `GET` more than 30 s after the grade writes them (never a second score); `retry` on that revision does the same immediately. The server setting `FEEDBACK_IN_BACKGROUND=false` restores the one-response behaviour.
